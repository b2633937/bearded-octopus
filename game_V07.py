...
def main():
    ...
    player2 = Player.new('Optimizer')

    AGENTS.append(Agent(player = player1, role='prey', nr=0, img=IMAGES['princess']))
    AGENTS.append(Agent(player = player2, role='predator', nr=1, img=IMAGES['boy']))
    AGENTS.append(Agent(player = player1, role='prey', nr=2, img=IMAGES['princess']))
    ...
    while True: #the main game loop
        ...
        if gameEnds(): #check for game end
            ...
            if rnd == rnds:
                ...
                try:
                    print 'saved'
                    pickle.dump(AGENTS[1].Qtable, open(str(episode) + AGENTS[1].QtableFN, "wb"), pickle.HIGHEST_PROTOCOL)
                    AGENTS[1].Qtable = {}
                except:
    ...
    if mode == 'random': #still prevent agents from getting same initial position
        ...
        for agent in AGENTS:
            ...
            while position in positions: 
                position = (random.randint(0,9), random.randint(0,9))
            agent.POS = position
    fixedInitPositions = [(5,5), (0,0), (10,10)]
    if mode == 'fixed':
